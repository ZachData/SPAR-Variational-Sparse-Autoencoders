...
def run_training(model, buffer, params):
    ...
    trainer_config = {
        ...
        "device": "cuda",
        "wandb_name": f"VSAEMulti_{MODEL_NAME}_d{DICT_SIZE}_lr{LR}_kl{KL_COEFF}_corr{N_CORRELATED_PAIRS}",
    ...
    # Set save directory
    save_dir = f"./trained_vsae_multi_{MODEL_NAME}_d{DICT_SIZE}_lr{LR}_kl{KL_COEFF}_corr{N_CORRELATED_PAIRS}_anticorr{N_ANTICORRELATED_PAIRS}"
...
def main():
    ...
    params = {
        ...
        # Dictionary size parameters - only using 4x
        "DICT_SIZE": 8192,  # 4x default d_mlp (2048)
        
        # Training parameters        
        "TOTAL_STEPS": 30000,  
        "LR": 5e-4,  # 0.0005
        "KL_COEFF": 50,  # Changed from 100 to 50
        ...
        "N_CORRELATED_PAIRS": 0.0,  # Changed to 0.0 to match naming convention
