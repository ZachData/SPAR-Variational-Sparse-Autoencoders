"""
Training script for Block Diagonal Laplacian TopK SAE (NO VAE) with sweep functionality.

This script implements training for the BDL-SAE-TopK (Block Diagonal Laplacian Sparse Autoencoder with TopK),
which is the SIMPLIFIED version without VAE components:

Key differences from VAE version:
✅ NO reparameterization trick
✅ NO KL divergence loss  
✅ NO variance encoder
✅ JUST: Standard AE + Top-K sparsity + Laplacian regularization

This enables clean ablation studies:
- Pure TopK SAE: ✓ (baseline)
- VSAE TopK: ✓ (adds VAE)  
- Laplacian TopK VAE: ✓ (adds VAE + Laplacian)
- Laplacian TopK SAE: ← THIS (adds just Laplacian, no VAE)

Mathematical Framework:
- Standard encoding: z = encoder(x) (deterministic)
- Top-K sparsity: Select k largest |z| values while preserving signs
- Laplacian regularization: R(z) = Σᵢ zᵢᵀLᵢzᵢ
...
# Import our LaplacianTopKSAE implementations
from dictionary_learning.trainers.laplace_topk_sae import (
    LaplacianTopKSAE, 
    LaplacianTopKSAETrainer,
    LaplacianTopKSAEConfig,
    LaplacianTopKSAETrainingConfig
)


@dataclass
class ExperimentConfig:
    """Configuration for the Block Diagonal Laplacian TopK SAE experiment (NO VAE)."""
    # Model configuration
    ...
    # Model-specific config (SIMPLIFIED - no VAE parameters)
    use_april_update_mode: bool = True
    
    # Training configuration (SIMPLIFIED - no KL parameters)
    total_steps: int = 10000
    lr: float = 5e-4
    auxk_alpha: float = 1/32  # TopK auxiliary loss coefficient
    
    # Schedule configuration (SIMPLIFIED - no KL annealing)
    ...
    wandb_project: str = "laplace-topk-sae-experiments"
    ...
    def __post_init__(self):
        """Set derived configuration values (SIMPLIFIED)."""
...
class ExperimentRunner:
    """Manages the entire training experiment with Block Diagonal Laplacian TopK SAE (NO VAE)."""
    ...
    def setup_logging(self) -> None:
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(log_dir / 'laplace_topk_sae_training.log'),
    ...
    def create_model_config(self, model: HookedTransformer) -> LaplacianTopKSAEConfig:
        ...
        # Create base config (SIMPLIFIED - no VAE parameters)
    ...
    def create_training_config(self) -> LaplacianTopKSAETrainingConfig:
        """Create training configuration from experiment config."""
        return LaplacianTopKSAETrainingConfig(
    ...
    def create_trainer_config(self, model_config: LaplacianTopKSAEConfig, training_config: LaplacianTopKSAETrainingConfig) -> Dict[str, Any]:
        """Create trainer configuration for the training loop."""
        trainer_config = {
            "trainer": LaplacianTopKSAETrainer,
    ...
    def get_experiment_name(self) -> str:
        ...
        return (
            f"LaplacianTopKSAE_{self.config.model_name}_"
            f"d{int(self.config.dict_size_multiple * 2048)}_"
            f"k{k_value}_lr{self.config.lr}_"
            f"aux{self.config.auxk_alpha}{laplacian_suffix}_no_vae"
        )
    ...
    def run_training(self) -> Dict[str, float]:
        """Run the complete training experiment with Block Diagonal Laplacian TopK SAE (NO VAE)."""
        self.logger.info("Starting Block Diagonal Laplacian TopK SAE training experiment (NO VAE)")
        ...
        try:
            ...
            self.logger.info("NO VAE COMPONENTS - simplified architecture")
            
            # Log block structure information
            dummy_model = LaplacianTopKSAE(model_config)
            block_info = dummy_model.laplacian.get_block_info()
            self.logger.info(f"Block structure: {block_info}")
            
            # Run training
            self.logger.info("Starting Block Diagonal Laplacian TopK SAE training...")
            trainSAE(
                data=buffer,
...
class LaplacianTopKSAESweepRunner(BaseSweepRunner):
    """
    Hyperparameter sweep runner for Block Diagonal Laplacian TopK SAE (NO VAE).
    
    Optimizes TopK and Laplacian-specific parameters without VAE complexity.
    """
    
    def __init__(self, wandb_entity: str):
        """Initialize Laplacian TopK SAE sweep runner."""
        super().__init__(trainer_name="laplace-topk-sae", wandb_entity=wandb_entity)
    
    def get_sweep_config(self) -> dict:
        """
        Define the wandb sweep configuration for Laplacian TopK SAE.
        
        Focuses on TopK and Laplacian parameters (NO VAE parameters).
        """
        return {
            'method': 'bayes',
            'metric': {
            ...
            'parameters': {
                ...
                # Laplacian regularization coefficient
                'laplacian_coeff': {
                    'distribution': 'log_uniform_values',
                    'min': 0.1,
                    'max': 10.0
                },
                # Laplacian type - discrete choice
                'laplacian_type': {
                ...
                # NO VAE parameters in this sweep!
            }
        }
    ...
    def get_run_name(self, sweep_params: dict) -> str:
        ...
        return f"SAE_lr{lr_str}_{k_str}_{aux_str}_{lap_str}_d{dict_str}_no_vae"
    ...
    def create_experiment_config(self, sweep_params: dict) -> ExperimentConfig:
        ...
        config.laplacian_coeff = sweep_params['laplacian_coeff']
        config.laplacian_type = sweep_params['laplacian_type']
        ...
        config.wandb_project = "laplace-topk-sae-sweeps"
...
def create_quick_test_config() -> ExperimentConfig:
    """Create a configuration for quick testing (SIMPLIFIED)."""
...
def create_full_config() -> ExperimentConfig:
    """Create a configuration for full training - memory efficient (SIMPLIFIED)."""
    return ExperimentConfig(
        model_name="gelu-1l",
        layer=0,
        hook_name="blocks.0.mlp.hook_post",
        ...
        # Full training parameters (SIMPLIFIED - no KL)
        total_steps=25000,
        lr=5e-4,
        auxk_alpha=1/32,
        
        # Model settings (SIMPLIFIED - no VAE)
...
def create_complete_graph_config() -> ExperimentConfig:
    """Create a configuration using complete graph Laplacian (SIMPLIFIED)."""
...
def create_ring_graph_config() -> ExperimentConfig:
    """Create a configuration using ring graph Laplacian (SIMPLIFIED)."""
    config = create_full_config()
...
def create_gpu_10gb_config() -> ExperimentConfig:
    """Create a configuration optimized for 10GB GPU memory (SIMPLIFIED)."""
    return ExperimentConfig(
        model_name="gelu-1l",
        layer=0,
        hook_name="blocks.0.mlp.hook_post",
        ...
        laplacian_type="complete",  # Most memory efficient
        laplacian_coeff=0.5,     # Slightly lower for stability
        
        # Training parameters optimized for 10GB GPU (SIMPLIFIED)
        total_steps=20000,
        lr=8e-4,
        auxk_alpha=1/32,
...
def create_high_regularization_config() -> ExperimentConfig:
    """Create a configuration with high Laplacian regularization (SIMPLIFIED)."""
    config = create_full_config()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Block Diagonal Laplacian TopK SAE Training (NO VAE)")
    parser.add_argument(
        "--sweep", 
        action="store_true", 
    ...
    if args.sweep:
        ...
        print("Starting hyperparameter sweep for Block Diagonal Laplacian TopK SAE (NO VAE)...")
        print(f"Project: laplace-topk-sae-sweeps")
        print(f"Entity: {args.wandb_entity}")
        
        sweep_runner = LaplacianTopKSAESweepRunner(wandb_entity=args.wandb_entity)
    ...
    if results:
        ...
        print("NOTE: This is the SIMPLIFIED version (NO VAE components)")
    else:
        print("No evaluation results available")


# Usage examples:
# python train_laplace_topk_sae.py --config quick_test
# python train_laplace_topk_sae.py --config complete_graph     # Use complete graph Laplacian
# python train_laplace_topk_sae.py --config ring_graph        # Use ring graph Laplacian  
# python train_laplace_topk_sae.py --config gpu_10gb          # Optimized for 10GB GPU
# python train_laplace_topk_sae.py --config high_regularization  # High Laplacian regularization
# python train_laplace_topk_sae.py --sweep --wandb-entity your-username  # Hyperparameter sweep
