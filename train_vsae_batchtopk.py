...
def create_gpu_10gb_config() -> ExperimentConfig:
    ...
    return ExperimentConfig(
        ...
        kl_coeff=30.0,  # Reduced KL for faster convergence
...
def create_gpu_10gb_speed_config() -> ExperimentConfig:
    ...
    return ExperimentConfig(
        ...
        kl_coeff=200.0,  # Lower KL for faster convergence
...
def main():
    ...
    if results:
        ...
        # Performance tip
        if args.config.startswith("gpu_10gb"):
            print(f"\n💡 Performance tip: If training was slow, try '--config gpu_10gb_speed' for maximum speed!")
    else:
