...
def run_training(config, model, buffer, device="cuda"):
    ...
    run_name = f"VSAEIso_{MODEL_NAME}_d{DICT_SIZE}_lr{LR}_kl{KL_COEFF}_warm{WARMUP_FRAC}_spwarm{SPARSITY_WARMUP_FRAC}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train VSAEIso models with hyperparameter sweep')
    ...
    if args.sweep_all:
        # Comprehensive sweep with more values
        sweep_params = {
            "LR": [5e-3, 1e-3, 5e-4, 1e-4],
            "KL_COEFF": [10, 50, 100, 200, 500],
            "WARMUP_FRAC": [0.01, 0.05, 0.1],
            "SPARSITY_WARMUP_FRAC": [0.01, 0.05, 0.1, 0.2],
            "DECAY_START_FRAC": [0.6, 0.8, 0.9],
            "VAR_FLAG": [0, 1],  # 0: fixed variance, 1: learned variance
            "USE_APRIL_UPDATE": [True, False]
        }
    else:
        # Limited sweep with fewer values (faster)
        sweep_params = {
            "LR": [5e-3, 1e-3, 5e-4, 1e-4],
            "KL_COEFF": [50, 100, 200],
            "WARMUP_FRAC": [0.05],
            ...
            "VAR_FLAG": [0, 1],  # Only fixed variance
