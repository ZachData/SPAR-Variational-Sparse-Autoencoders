...
def parse_feature_data(
    ...
    try:
        # Try the original einsum operation
        logits = einops.einsum(
            feature_resid_dir, model.W_U, "feats d_model, d_model d_vocab -> feats d_vocab"
        )
    except RuntimeError as e:
        # Handle dimension mismatch gracefully
        print(f"Warning: Dimension mismatch in einsum operation: {e}")
        print(f"feature_resid_dir shape: {feature_resid_dir.shape}, model.W_U shape: {model.W_U.shape}")
        
        # Method 1: Project the feature directions to the correct dimension
        d_model = model.W_U.shape[0]
        d_vocab = model.W_U.shape[1]
        n_features = feature_resid_dir.shape[0]
        
        if feature_resid_dir.shape[1] > d_model:
            # If feature_resid_dir is larger, truncate it
            projected_directions = feature_resid_dir[:, :d_model]
        else:
            # If feature_resid_dir is smaller, pad with zeros
            projected_directions = torch.zeros(n_features, d_model, device=feature_resid_dir.device)
            projected_directions[:, :feature_resid_dir.shape[1]] = feature_resid_dir
        
        # Try again with the projected directions
        try:
            logits = einops.einsum(
                projected_directions, model.W_U, "feats d_model, d_model d_vocab -> feats d_vocab"
            )
            print(f"Successfully projected and computed logits with shape {logits.shape}")
        except Exception as proj_error:
            # If projection still fails, fall back to random values
            print(f"Projection failed: {proj_error}. Using random fallback logits.")
            logits = torch.randn(n_features, d_vocab, device=feature_resid_dir.device) * 0.01
            print(f"Created fallback logits with shape {logits.shape}")
    
    # Create probe_names_and_values similarly with appropriate error handling
    probe_names_and_values = []
    for mode, name, probe in linear_probes:
        try:
            if mode == "output":
                probe_logits = einops.einsum(
                    feature_resid_dir, probe, "feats d_model, d_model d_vocab_out -> feats d_vocab_out"
                )
            else:  # mode == "input"
                probe_logits = einops.einsum(
                    feature_resid_dir_input, probe, "feats d_model, d_model d_vocab_out -> feats d_vocab_out"
                )
            probe_names_and_values.append((f"PROBE {name!r}, {mode.upper()} SPACE", probe_logits))
        except RuntimeError as e:
            print(f"Warning: Skipping probe {name} due to dimension mismatch: {e}")
            # Create fallback probe logits
            d_vocab_out = probe.shape[1]
            fallback_logits = torch.randn(len(feature_indices), d_vocab_out, device=feature_resid_dir.device) * 0.01
            probe_names_and_values.append((f"PROBE {name!r}, {mode.upper()} SPACE (FALLBACK)", fallback_logits))
    ...
    if layout.seq_cfg is not None:
        for i, feat in enumerate(feature_indices):
            try:
                ...
                print(f"Warning: Error generating sequence data for feature {feat}: {e}")
                # Create empty sequence data as fallback
                feature_data_dict[feat]["seqMultiGroup"] = SeqMultiGroupData()
...
def get_sequences_data(
    ...
    """
    This function returns the data which is used to create the top activating sequence visualizations.
    """
    ...
    # Check if the required hooks are available in the cache
    required_hooks = [resid_final_hook_name, acts_post_hook_name]
    missing_hooks = [hook for hook in required_hooks if hook not in cache.cache_dict]
    if missing_hooks:
        print(f"Warning: Missing required hooks: {missing_hooks}")
        print(f"Available hooks: {list(cache.cache_dict.keys())}")
        print("Creating empty sequence data as fallback")
        return SeqMultiGroupData()
    ...
    # We define our full buffer going 1 token further back than all the visible tokens.
    ...
    if feat_acts.ndim < 2:
        print(f"Warning: feat_acts has unexpected shape {feat_acts.shape}, reshaping")
        # Reshape to expected dimensions if needed
        feat_acts = feat_acts.view(1, -1)
    
    # Calculate max activation with proper error handling
    try:
        feat_acts_max = feat_acts[
            :, buffer_to_exclude_from_ex[0] : buffer_to_exclude_from_ex[1]
        ].max()
    except Exception as e:
        print(f"Warning: Error calculating feat_acts_max: {e}")
        print(f"feat_acts shape: {feat_acts.shape}")
        # Fallback to just using max of all activations
        feat_acts_max = feat_acts.max()

    # Get the top-activating tokens with error handling
    try:
        indices = k_largest_indices(
            feat_acts,
            k=min(seq_cfg.top_acts_group_size, feat_acts.numel()),  # Don't request more than we have
            buffer=buffer_to_exclude_from_ex,
        )
    except Exception as e:
        print(f"Warning: Error finding top activations: {e}")
        # Fallback to a simple argmax approach
        flat_indices = torch.flatten(feat_acts).argsort(descending=True)[:min(seq_cfg.top_acts_group_size, feat_acts.numel())]
        if feat_acts.ndim > 1:
            # Convert flat indices to 2D
            rows = flat_indices // feat_acts.shape[1]
            cols = flat_indices % feat_acts.shape[1]
            indices = torch.stack([rows, cols], dim=1)
        else:
            # Handle 1D case
            indices = flat_indices.unsqueeze(1)
    ...
    if seq_cfg.n_quantiles > 0:
        try:
            ...
            for i in range(seq_cfg.n_quantiles - 1, -1, -1):
                ...
                pct = ((feat_acts >= lower) & (feat_acts <= upper)).float().sum() / max(n_active, 1)  # Avoid div by 0
                try:
                    indices = random_range_indices(
                        feat_acts,
                        k=min(seq_cfg.quantile_group_size, ((feat_acts >= lower) & (feat_acts <= upper)).sum().item()),
                        bounds=(lower, upper),
                        buffer=buffer_to_exclude_from_ex,
                    )
                    indices_dict[
                    ...
                    print(f"Warning: Error creating quantile group {i}: {e}")
                    # Skip this quantile group
                    continue
        except Exception as e:
            print(f"Warning: Error creating quantiles: {e}")
            # Don't add quantile groups if there was an error

    # Check if we have any valid indices
    if not indices_dict:
        print("Warning: No valid sequence indices found")
        return SeqMultiGroupData()
    ...
    try:
        ...
        print(f"Warning: Error concatenating indices: {e}")
        # Try to fix indices if needed
        fixed_indices = []
        for group_indices in indices_dict.values():
            if isinstance(group_indices, torch.Tensor) and group_indices.ndim == 2:
                fixed_indices.append(group_indices)
        if not fixed_indices:
            print("Warning: No valid indices to concatenate")
            return SeqMultiGroupData()
        indices_ex = torch.concat(fixed_indices).cpu()
    ...
    try:
        ...
        print(f"Warning: Error indexing token sequences: {e}")
        # Create dummy token IDs as fallback
        token_ids = torch.zeros(n_ex, 5, dtype=torch.long)
        token_ids_to_display = token_ids
    ...
    try:
        ...
        print(f"Warning: Error extracting feature activations: {e}")
        # Create dummy values as fallback
        feat_acts_pre_ablation = torch.zeros(n_ex, 1, device=device)
        feat_acts_coloring = feat_acts_pre_ablation
        feat_acts_idx = [None for _ in range(n_ex)]
        resid_post = torch.zeros(n_ex, 1, feature_resid_dir.shape[0], device=device)
        token_ids_for_computing_loss = torch.zeros(n_ex, 1, dtype=torch.long, device=device)

    # ! (4) Compute the logit effect if this feature is ablated
    try:
        # Check for dimension compatibility
        if resid_post.shape[-1] != feature_resid_dir.shape[0]:
            print(f"Warning: Dimension mismatch between resid_post ({resid_post.shape[-1]}) and feature_resid_dir ({feature_resid_dir.shape[0]})")
            # Project the feature direction tensor to match residual stream
            d_model = resid_post.shape[-1]
            if feature_resid_dir.shape[0] > d_model:
                # If feature_resid_dir is larger, truncate it
                feature_resid_dir_padded = feature_resid_dir[:d_model]
            else:
                # If feature_resid_dir is smaller, pad with zeros
                feature_resid_dir_padded = torch.zeros(d_model, device=feature_resid_dir.device)
                feature_resid_dir_padded[:feature_resid_dir.shape[0]] = feature_resid_dir
            ...
            resid_post_feature_effect = einops.einsum(
                feat_acts_pre_ablation,
                feature_resid_dir_padded.to(device),
        ...
        try:
            ...
            print(f"Warning: Error computing logit contribution: {e}")
            print(f"resid_post_feature_effect shape: {resid_post_feature_effect.shape}, W_U shape: {model.W_U.shape}")
            # Create dummy logit contribution
            logit_contribution = torch.zeros(resid_post_feature_effect.shape[0], resid_post_feature_effect.shape[1], model.W_U.shape[1], device=device)
        ...
        try:
            ...
            print(f"Warning: Error computing logprobs contribution: {e}")
            # Create dummy values
            d_vocab = model.W_U.shape[1]
            logprobs_contribution = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], d_vocab, device=device)
            orig_prob = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], d_vocab, device=device)
            new_prob = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], d_vocab, device=device)
        ...
        try:
            ...
            print(f"Warning: Error computing top/bottom logits: {e}")
            # Create dummy top/bottom values
            d_vocab = logprobs_contribution.shape[-1]
            dummy_values = torch.zeros(n_ex, seq_cfg.top_logits_hoverdata, device=device)
            dummy_indices = torch.zeros(n_ex, seq_cfg.top_logits_hoverdata, dtype=torch.long, device=device)
            top_logit_contribution = namedtuple('TopK', ['values', 'indices'])(dummy_values, dummy_indices)
            bottom_logit_contribution = namedtuple('TopK', ['values', 'indices'])(dummy_values, dummy_indices)

        if target_logits is None:
            # Handle potential indexing errors
            try:
                ...
                print(f"Warning: Error indexing logprobs: {e}")
                # Create dummy values
                loss_contribution = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], device=device)
                logit_contribution = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], device=device)
                orig_prob = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], device=device)
                new_prob = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], device=device)
        else:
            ...
            try:
                ...
                print(f"Warning: Error computing cross entropy loss: {e}")
                # Create dummy loss
                loss_contribution = torch.zeros(n_ex, feat_acts_pre_ablation.shape[1], device=device)
            logit_contribution = orig_prob = new_prob = None
    except Exception as e:
        print(f"Warning: Error computing logit effects: {e}")
        # Create dummy values for everything
        d_vocab = model.W_U.shape[1]
        logit_contribution = torch.zeros(n_ex, 1, device=device)
        loss_contribution = torch.zeros(n_ex, 1, device=device)
        orig_prob = new_prob = None
        top_logit_contribution = namedtuple('TopK', ['values', 'indices'])(
            torch.zeros(n_ex, seq_cfg.top_logits_hoverdata, device=device),
            torch.zeros(n_ex, seq_cfg.top_logits_hoverdata, dtype=torch.long, device=device)
        )
        bottom_logit_contribution = namedtuple('TopK', ['values', 'indices'])(
            torch.zeros(n_ex, seq_cfg.top_logits_hoverdata, device=device),
            torch.zeros(n_ex, seq_cfg.top_logits_hoverdata, dtype=torch.long, device=device)
        )
    ...
    if use_dfa:
        try:
            ...
            print(f"Warning: Error computing DFA: {e}")
            dfa_token_ids = dfa_values = indices_ex_src = None
    ...
    try:
        ...
        dfa_buffer_range = (
            ...
            if seq_cfg.dfa_buffer is not None and dfa_token_ids is not None
            else range(tokens.shape[1]) if dfa_token_ids is not None
            else []
        )
        ...
        for group_idx, group_name in enumerate(indices_dict.keys()):
            try:
                ...
                print(f"Warning: Error creating sequence group {group_idx}: {e}")
                # Skip this group
    except Exception as e:
        print(f"Warning: Error creating sequence groups: {e}")
        # Create an empty group
        sequence_groups_data.append(SeqGroupData([], title="Error creating sequences"))
