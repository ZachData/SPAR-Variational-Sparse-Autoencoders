...
def run_training(config, model, buffer, device="cuda"):
    ...
    # VSAEMixture specific parameters
    n_correlated_pairs = config.get("N_CORRELATED_PAIRS", 0)
    n_anticorrelated_pairs = config.get("N_ANTICORRELATED_PAIRS", 0)
    ...
    run_name = f"VSAEMix_{MODEL_NAME}_d{DICT_SIZE}_lr{LR}_kl{KL_COEFF}_corr{n_correlated_pairs}_anticorr{n_anticorrelated_pairs}"
    ...
    trainer_config = {
        ...
        "var_flag": config.get("VAR_FLAG", 0),  # Default to fixed variance
        "use_april_update_mode": config.get("USE_APRIL_UPDATE", True),
    ...
    main_save_dir = "./trained_sweeps_mixture_8x"
    ...
    run_cfg = {
        ...
        "experiment_type": "mixturevsae_sweep",
        ...
        "use_april_update": config.get("USE_APRIL_UPDATE", True),
        "n_correlated_pairs": n_correlated_pairs,
        "n_anticorrelated_pairs": n_anticorrelated_pairs,
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train VSAEMixtureGaussian models with hyperparameter sweep')
    ...
    if args.sweep_all:
        # Comprehensive sweep with more values for mixture
        sweep_params = {
            ...
            "USE_APRIL_UPDATE": [True, False],
            "N_CORRELATED_PAIRS": [0, 10, 50, 100],
            "N_ANTICORRELATED_PAIRS": [0, 10, 50, 100]
        }
    else:
        # Limited sweep with fewer values for mixture (faster)
        sweep_params = {
            "LR": [1e-3, 5e-4],
            "KL_COEFF": [50, 100, 200],
            "WARMUP_FRAC": [0.05],
            ...
            "USE_APRIL_UPDATE": [True],  # Only April update mode
            "N_CORRELATED_PAIRS": [0, 50],
            "N_ANTICORRELATED_PAIRS": [0, 50]
        }
    ...
    print("\n\n" + "="*50)
    print("COMPARATIVE RESULTS ACROSS ALL RUNS FOR VSAEMIXTURE")
    print("="*50)
