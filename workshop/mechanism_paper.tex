...
them stated in any equation. We close with a case study (SAEBench's SCR and
...
\item A lower bound on implementation variance in this literature: five
  ...
  $d=19.3$ for the single largest, at $5\sigma$ (\S\ref{sec:variance}).
...
already, two were static no-ops confirmed by reading (never reachable, or
algebraically inert given the training configuration), seven are further
static no-ops of the same kind, one (the RNG consumption order) is treated as
noise by the seed-permutation design itself, and five had to be run as
measured factors. \emph{Exactly one} of the fifteen is a difference in the
...
k/d$) are reported per CLAUDE.md's two-threshold rule; \ns\ marks a
non-significant result at $p>0.05$ (13 seeds/arm, $p$-floor $2.5\times
10^{-7}$, $5.03$--$5.16\sigma$).}
\label{tab:e1ladder}
\begin{tabular}{lccccc}
\toprule
vSAE arm & factors matched (cumulative) & FVE ($d$) & frac.\ recovered ($d$) & $<0.1\times$ ($d$) & $<0.5\times$ ($d$) \\
\midrule
...
\begin{description}
\item[Decoder-gradient projection.] \texttt{vsae\_topk.py} imports
  \texttt{remove\_gradient\_parallel\_to\_decoder\_directions} and never calls
  it, while still renormalising the decoder to unit norm every step --- the
...
Of the $0.4410$ total gap, removing the KL recovers only $6.4\%$; the
...
($\log\sigma^2_{\mathrm{init}} \in \{-1,-2,-3,-4,-5,-8\}$, the last clamped to
...
$-2.0$ & $0.3679$ & $0.4841 \pm 0.0022$ & $0.8117 \pm 0.0013$ \\
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{a2_dose_response.pdf}
\caption{TopK: reconstruction quality (FVE, left axis) and selection
stability (Jaccard, right axis) as a function of the reparameterisation noise
scale, 13 seeds/point. The two curves are visually near-coincident;
$r=+0.9993$.}
\label{fig:a2}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{a3_batchtopk_dose_response.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{a4_jumprelu_dose_response.pdf}
\caption{JumpReLU: the same sweep. The naive $r=+0.9334$ annotated on this
figure is confounded (\S\ref{sec:a4confound}); it is superseded by the
$L_0$-controlled reading in the text.}
\label{fig:a4}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{a4_followup_dose_response.pdf}
...
explanation that addendum 20's original reading was a fluke of only four
...
\midrule
$<0.1\times k/d$ & $0.1309 \pm 0.0060$ & $0.0298 \pm 0.0038$ ($d=+20.8$) \\
$<0.5\times k/d$ & $0.4654 \pm 0.0060$ & $0.3134 \pm 0.0093$ ($d=+19.8$) \\
\bottomrule
\end{tabular}
\end{table}
...
$\sim4.4$ at the tight threshold. Had either trainer simply been patched to
...
would have silently inherited a $d\approx20$ effect misattributed to whatever
...
reported without seed variance at all (\S\ref{sec:e4discussion}); a $d>10$
...
\texttt{F.relu(mu)} presence/absence & liveness & $19.8$--$20.8$ \\
\bottomrule
\end{tabular}
...
\subsection{The decoder-gradient-projection effect does not generalise to a plain, penalty-free Top-K SAE}
\label{sec:claim4}
...
at $n=6$ per group (a seed count we observed informally to be common in
this literature) that floor is $p=0.00216$, or $3.07\sigma$ --- not because
the effect is small, but because the design cannot express more evidence
than that, however large $d$ actually is. Thirteen seeds per group is the
first $n$ at which this floor drops below the conventional $5\sigma$
threshold. Every effect size in this paper is real in the sense that it
survived a design capable of expressing $5\sigma$; the implementation
choices catalogued in Table~\ref{tab:variance} are large enough that most
published single-seed or few-seed comparisons in this space could not, even
in principle, distinguish them from the architectural effect the comparison
was nominally about.
...
correlation removal (SCR) and targeted probe perturbation (TPP) metrics on
the same dataset (\texttt{bias\_in\_bios}, professor/nurse) and the same
...
(unmasked, natural) score against the resulting curve at $N=1{,}474$.
...
\item \textbf{Is the masked reference curve itself the wrong comparison?}
  ...
  directly finds its SCR score ($0.020127$) matches the masked curve's
  reference ($0.020127$) to $6\times10^{-8}$, and its TPP score ($0.2025$)
  is slightly \emph{higher} than the masked reference ($0.1905$). Both
  verdicts survive unchanged (SCR) or are marginally reinforced (TPP): this
  specific confound is not live. (A secondary finding en route: even given
  the vSAE's exact live-feature count as its budget, a plain Top-K network
  trained at that size keeps only $65.5\%$ of it alive --- matching a
  feature \emph{count} does not guarantee matching how efficiently a network
  uses it.)
\end{enumerate}
...
discrete choices generally), not in the magnitude of an already-selected
feature. A prior with mass at zero --- spike-and-slab being the textbook
example for sparse codes, in contrast to the isotropic Gaussian's zero mass
at the origin --- is a second, related direction: it would make the KL term
itself do something other than shrink activation magnitude uniformly.
Neither is tested here; both follow directly from identifying the specific
mechanism this paper measures rather than only its symptom.
...
\citet{bricken2023monosemanticity}; Top-K sparsity, which this paper's
baseline uses throughout, is due to
...
a confound. BatchTopK~\citep{bussmann2024batchtopk} and
JumpReLU~\citep{rajamanoharan2024jumprelu} are the two alternative selection
mechanisms this paper's \S\ref{sec:mechanism} tests against Top-K's own
noise-robustness. SAEBench~\citep{karvonen2025saebenchcomprehensivebenchmarksparse}
is the source of the SCR and TPP metrics scored in \S\ref{sec:e4}, and of the
sparse feature circuit evaluation methodology this codebase's downstream
tooling is built on~\citep{marks2025sparsefeaturecircuitsdiscovering}. The
...
threshold trainable~\citep{bengio2013estimating}, and the two continuous
...
are the closest prior treatments of the specific tension
\S\ref{sec:mechanism} measures: that a continuous relaxation of a discrete
choice and a discrete choice made from continuously-perturbed inputs are not
the same operation, and only one of them is what a sampled sparse code
actually needs. We are not aware of a prior controlled decomposition of a
variational sparse autoencoder into its constituent implementation choices,
of a dose-response measurement of the reparameterisation-selection
interaction across multiple selection mechanisms, or of a reported
disagreement between SCR and TPP of the kind \S\ref{sec:e4} documents.
...
\texttt{python falsification/run\_arm.py \texttt{-}\texttt{-}arm <name>
\texttt{-}\texttt{-}seed <seed>}; \texttt{falsification/preflight.py} validates
every arm's wiring without requiring a GPU. \S\ref{sec:e1}'s arms are
\texttt{e1\_penalty}, \texttt{e1\_vsae\_ref\{,\_unitinit,\_gradproj,\_fullmatch\}};
\S\ref{sec:mechanism}'s are \texttt{a2\_sigma\_init\_\{m1,m3,m4,m5\}} plus
\texttt{e2\_sampling\_only} and \texttt{e2\_sigma\_low\_init} (TopK),
\texttt{a3\_batchtopk\_*} (BatchTopK), and \texttt{a4\_jumprelu\_*}
(JumpReLU); \S\ref{sec:variance}'s ReLU factor is
\texttt{e3\_masked\_kl}/\texttt{e3\_masked\_kl\_relu}, and its
projection-generalisation control (\S\ref{sec:claim4}) is \texttt{baseline}
vs.\ \texttt{claim4\_baseline\_noproj}. Every arm ran at 13 seeds, except
\S\ref{sec:a4followup}'s four densifying JumpReLU points
(\texttt{a4\_jumprelu\_sigma\_init\_\{m3\_5,m4\_5,m5\_5,m6\}}), which ran at
5 (\texttt{falsification/read\_a4\_followup.py}). The falsification test suite
(\texttt{python -m pytest falsification/tests/ -q}) exercises the
permutation-test implementation itself, including the $p$-value floor
formulas in \S\ref{app:stats}, and stayed green throughout this project.
