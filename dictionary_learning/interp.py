...
def feature_tsne(
    ...
    metric="euclidean", 
    random_state=42,
    n_components=2,  # dimension of the t-SNE embedding
    feat_idxs=None,  # if not none, indicate the feature with a red dot
):
    """
    Fit a t-SNE embedding of the dictionary features and return a plotly plot of the result.
    Works with both nnsight and transformer_lens models.
    
    Parameters:
        dictionary: The dictionary model to visualize
        weight: Which weights to use ('encoder' or 'decoder')
        ...
        feat_idxs: Feature indices to highlight in red
    ...
    # Extract weights from dictionary (same code as feature_umap)
    if weight == "encoder":
        if hasattr(dictionary, 'encoder') and hasattr(dictionary.encoder, 'weight'):
            df = pd.DataFrame(dictionary.encoder.weight.cpu().detach().numpy())
        elif hasattr(dictionary, 'W_enc'):
            df = pd.DataFrame(dictionary.W_enc.cpu().detach().numpy())
        else:
            raise ValueError(f"Unsupported dictionary structure for weight='{weight}'")
    else:  # weight == "decoder"
        if hasattr(dictionary, 'decoder') and hasattr(dictionary.decoder, 'weight'):
            df = pd.DataFrame(dictionary.decoder.weight.T.cpu().detach().numpy())
        elif hasattr(dictionary, 'W_dec'):
            df = pd.DataFrame(dictionary.W_dec.T.cpu().detach().numpy())
        else:
            raise ValueError(f"Unsupported dictionary structure for weight='{weight}'")
    ...
    if n_components == 2:
        fig = px.scatter(
            ...
            hover_name=df.index, 
            color=colors,
            title="t-SNE visualization of dictionary features"
        )
        fig.update_layout(
        ...
        fig = px.scatter_3d(
            ...
            hover_name=df.index,
            color=colors,
            title="t-SNE visualization of dictionary features (3D)"
        )
        fig.update_layout(
            scene=dict(
                xaxis_title="t-SNE dimension 1",
