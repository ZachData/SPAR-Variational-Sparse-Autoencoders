...
class ComprehensiveLatentAnalyzer:
    ...
    def _save_feature_tsne_plots(self, data: LatentSpaceData, results: Dict[str, Any]) -> None:
        ...
        axes[0, 0].set_xlabel('Feature t-SNE 1')
        axes[0, 0].set_ylabel('Feature t-SNE 2')
        axes[0, 0].set_title(f'Feature Clusters\n{data.model_name}\n'
        ...
        axes[0, 1].set_xlabel('Feature t-SNE 1')
        axes[0, 1].set_ylabel('Feature t-SNE 2')
        axes[0, 1].set_title('Feature Utilization Rate')
        plt.colorbar(scatter, ax=axes[0, 1], label='Utilization Rate')
        ...
        axes[1, 0].set_xlabel('Feature t-SNE 1')
        axes[1, 0].set_ylabel('Feature t-SNE 2')
        axes[1, 0].set_title('Features: Color=Mean Activation, Size=Utilization')
        plt.colorbar(scatter, ax=axes[1, 0], label='Mean Activation')
        ...
        bars = axes[1, 1].bar(cluster_ids, cluster_utils, 
                             width=0.8, alpha=0.7, 
        ...
        axes[1, 1].set_ylabel('Mean Utilization Rate')
        axes[1, 1].set_title('Feature Cluster Utilization')
        axes[1, 1].set_ylim(0, max(cluster_utils) * 1.2)
        
        plt.tight_layout()
        feature_tsne_file = self.output_dir / f"feature_tsne_{data.model_name.replace('/', '_')}.png"
        plt.savefig(feature_tsne_file, dpi=150, bbox_inches='tight')
        plt.close()
