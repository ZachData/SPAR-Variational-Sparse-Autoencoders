...
def run_training(dict_size_multiple, model, buffer, base_params):
    ...
    trainer_config = {
        ...
        "device": "cuda",
        "wandb_name": f"VSAEGatedAnneal_{MODEL_NAME}_{DICT_SIZE}_lr{LR}_kl{KL_COEFF}_warm{WARMUP_STEPS}_spwarm{SPARSITY_WARMUP_STEPS}",
    ...
    save_dir = f"./VSAEGatedAnneal_{MODEL_NAME}_d{DICT_SIZE}_lr{LR}_kl{KL_COEFF}_warm{WARMUP_STEPS}_spwarm{SPARSITY_WARMUP_STEPS}"
    ...
    trainSAE(
        ...
        run_cfg={
            ...
            "experiment_type": "gated_anneal_vsae",
...
def main():
    ...
    base_params = {
        ...
        "ANNEAL_START": 2000,         # When to start annealing p
        "ANNEAL_END": 7000,           # When to end annealing p
        "P_START": 1.0,               # Initial p value
        "P_END": 0.05,                # Final p value
        "N_SPARSITY_UPDATES": 10,     # Number of p updates during annealing
